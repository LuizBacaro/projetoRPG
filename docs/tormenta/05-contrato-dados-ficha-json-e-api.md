...
- `reliquias[]` — itens mágicos HA (nome, efeito_resumo, slug opcional); área **Relíquias** na ficha, separada do inventário mundano
- `armaduras_protecao[]` — proteção equipada (CA); efeitos estruturados em catálogo HA quando aplicável
...
- `POST /api/v1/tormenta/regras/efeitos/agregar` — agrega bônus/penalidades de origem, poderes e itens (`ficha_json`, `poderes_slugs`, `contexto`). Response: `fontes`, `totais`, `condicionais`, `ativos`.
- `GET /api/v1/tormenta/regras/origens?suplemento=herois_arton` — inclui as 30 origens HA. As origens HA vêm com `beneficio_fixo: true`: o benefício é **único e automático** (livro, p. 46), não há "escolha 2". Todas as perícias de `beneficios_pericias` são treinadas automaticamente e concedem vaga extra no orçamento. `poderes_escolher` (ex.: Menestrel = 1) indica quantos poderes de `beneficios_poderes` o jogador escolhe; quando ausente, todos são automáticos. O backend completa `ficha_json.origem_beneficios` com os benefícios automáticos, então o cliente só precisa enviar a escolha opcional de poder.
- `POST /api/v1/tormenta/personagens/{id}/habilidades-origem/ativar` — ativa uma habilidade acionável da origem. Payload: `{ "habilidade_id": "..." }`. O servidor confere se a habilidade pertence à origem atualmente salva, usa o custo de PM do catálogo (o cliente não pode sobrescrevê-lo), rejeita saldo insuficiente e devolve `pa_atual_antes`, `pa_atual_depois`, `pa_max`, frequência, duração e resumo. Na ficha, usos `1×/cena` e efeitos com duração de cena ficam marcados localmente até o usuário acionar **Nova cena**; PM é sempre persistido pelo backend.
- `GET /api/v1/tormenta/regras/reliquias` — catálogo de itens mágicos HA (Cap. 3).
- `GET /api/v1/tormenta/regras/equipamentos?suplemento=herois_arton` — inclui arsenal HA.
