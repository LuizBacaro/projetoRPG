/**
 * Popup de solicitações pendentes para mestre — dashboard Tormenta.
 */
(function (global) {
    let pollTimer = null;
    let fila = [];
    ...
    async function carregarPendentes(campSvc) {
    ...
    global.T20CampanhaSolicitacoesMestre = {
        async verificar(opts) {
            ...
            try {
                const rows = await carregarPendentes(campSvc);
                if (!rows.length) return;
                fila = rows.slice();
                void mostrarProxima(campSvc, Toast, onAceita);
            } catch (_e) {
                /* silencioso — mestre sem campanhas ou API indisponível */
            }
        },
