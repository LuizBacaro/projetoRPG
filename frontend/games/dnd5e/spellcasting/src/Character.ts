...
export class SpellCasterCharacter {
  ...
  private readonly abilityScores: Record<Ability, number>;
  ...
  constructor(options: SpellCasterCharacterOptions) {
    ...
    this.spellSlotsManager = new SpellSlotsManager(
      this.level,
      this.classSlug,
      options.slotsUsed
    );
    ...
    this.spellBook = new SpellBook(
      this.classSlug,
      options.resolveSpell,
      options.spellBookState
    );
  }
  ...
  hasMaterialComponent(spell: Spell): boolean {
    ...
    if (materialCost > 0) {
      const gold = this.inventory.reduce(
        (s, i) => s + (i.goldValue ?? 0) * i.quantity,
        0
      );
    ...
    if (materialConsumed) {
      return this.inventory.some(
        (i) => i.name.toLowerCase().includes(material.toLowerCase().slice(0, 8)) && i.quantity > 0
      );
    }
  ...
  consumeMaterialComponent(spell: Spell): void {
    ...
    if (!material || !materialConsumed) return;
    const item = this.inventory.find((i) =>
      i.name.toLowerCase().includes(material.toLowerCase().slice(0, 8))
    );
