...
import {
  ...
  slotsForCharacterLevel,
} from './spellSlotsTable.js';

export class SpellSlotsManager {
  /** Slots máximos por índice de nível (0 = truque, sem gasto). */
  private readonly maxByLevel: number[];

  /** Slots já gastos por nível. */
  private usedByLevel: number[];

  private readonly classSlug: string;
  ...
  upcastToSlot(spellLevel: number, slotLevel: number): void {
    ...
    if (slotLevel < spellLevel) {
      throw new Error(
        `Slot de ${slotLevel}º nível é inferior ao nível da magia (${spellLevel}º).`
      );
    }
