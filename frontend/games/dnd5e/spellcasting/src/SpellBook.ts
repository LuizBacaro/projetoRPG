...
const EMPTY_STATE: SpellBookState = {
  mode: 'known',
  spellbookIds: [],
  knownSpellIds: [],
  preparedSpellIds: [],
...
export function spellListModeForClass(classSlug: string): SpellListMode {
...
export class SpellBook {
  ...
  private readonly resolveSpell: (id: string) => Spell | undefined;

  constructor(
    classSlug: string,
    resolveSpell: (id: string) => Spell | undefined,
    initial?: Partial<SpellBookState>
  ) {
  ...
  prepare(spellId: string, maxPrepared: number): boolean {
    if (this.state.preparedSpellIds.includes(spellId)) return true;
    if (this.state.preparedSpellIds.length >= maxPrepared) return false;
    const inBook =
      this.state.spellbookIds.includes(spellId) ||
   ...
   */
  static maxPreparedCount(
    mode: SpellListMode,
    abilityMod: number,
    casterLevel: number
  ): number {
    if (mode !== 'prepared') return 0;
