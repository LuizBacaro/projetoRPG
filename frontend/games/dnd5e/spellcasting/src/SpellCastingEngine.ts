...
import {
  ...
  SpellTargetCharacter,
} from './Character.js';
import {
  buildDiceExpression,
  cantripDamageDiceCount,
  parseDieFaces,
  rollDiceExpression,
} from './dice.js';
...
export class SpellCastingEngine {
  ...
  castSpell(
    ...
    target?: SpellTargetCharacter | Point,
    ...
    if (
      spell.castingTime.unit === 'bonus_action' &&
    ...
    if (
      ...
      return {
        success: false,
        reason:
          'Após magia de ação bônus, só truques podem ser lançados como ação neste turno.',
      };
    }
    ...
    const attackBonus =
      caster.spellcastingAbilityModifier + caster.proficiencyBonus;

    const targetChar =
      target && 'armorClass' in target
        ? (target as SpellTargetCharacter)
        : undefined;
  ...
  private applyEffect(
    spell: Spell,
    caster: SpellCasterCharacter,
    target: SpellTargetCharacter | undefined,
    slotLevel: number,
    dc: number,
    attackBonus: number
  ): EffectResult {
    const result: EffectResult = { dc };
  ...
  rollSpellDamage(
    ...
    caster: SpellCasterCharacter,
    slotLevel: number,
    roller: DiceRoller = caster.roller
  ): { total: number; expression: string } {
    const info = spell.damageInfo;
    if (!info) return { total: 0, expression: '0' };
