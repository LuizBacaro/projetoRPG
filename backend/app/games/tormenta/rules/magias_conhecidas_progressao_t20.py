...
def classe_usa_limite_conhecidas_mb(slug_classe: str) -> bool:
    """Espontâneo com tabela de conhecidas (bardo, feiticeiro)."""
    if modo_conjuracao_classe_mb(slug_classe) != "espontaneo":
        return False
    return _classe_row(slug_classe) is not None


def tabela_conhecidas_por_nivel_mb(slug_classe: str, nivel: int) -> Optional[List[int]]:
...
def limite_magias_conhecidas_circulo_mb(
    slug_classe: str, nivel: int, circulo: int
) -> Optional[int]:
    tab = tabela_conhecidas_por_nivel_mb(slug_classe, nivel)
    if tab is None:
        return None
    try:
        c = int(circulo)
...
def total_magias_conhecidas_max_mb(slug_classe: str, nivel: int) -> Optional[int]:
    tab = tabela_conhecidas_por_nivel_mb(slug_classe, nivel)
    if tab is None:
        return None
    return int(sum(tab))
...
def validar_adicionar_conhecida_mb(
    *,
    ...
    if not classe_usa_limite_conhecidas_mb(slug_classe):
        return True, ""
    try:
    ...
    lim = limite_magias_conhecidas_circulo_mb(slug_classe, nivel, circ)
    if lim is None:
        return True, ""
...
def preview_magias_conhecidas_mb(
    *,
    ...
    tab = tabela_conhecidas_por_nivel_mb(slug_classe, nivel)
    usa = classe_usa_limite_conhecidas_mb(slug_classe)
    ...
    total_max = total_magias_conhecidas_max_mb(slug_classe, nivel) if usa else None
    por_circulo = []
    if tab:
