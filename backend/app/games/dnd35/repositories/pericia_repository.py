...
class PericiaRepository:
    ...
    def restaurar_pericia(
        db: Session, db_pericia: Pericia, pericia: PericiaCreate
    ) -> Pericia:
    ...
    @staticmethod
    def listar_pericias_por_classe(
        db: Session, classe_nome: str
    ) -> list[Pericia]:
    ...
    @staticmethod
    def obter_custo_pericia(
        db: Session, pericia_id: int, classe_nome: str
    ) -> int:
        """
    ...
    def atualizar_pericia(
        db: Session, pericia_id: int, pericia: PericiaUpdate
    ) -> Pericia:
...
class PericiaJogadorRepository:
    ...
    def adicionar_pericia(
        db: Session, pericia_jogador: PericiaJogadorCreate
    ) -> PericiaJogador:
    ...
    def obter_pericia_jogador(
        db: Session, pericia_jogador_id: int
    ) -> PericiaJogador:
        """Obtém uma perícia específica do jogador"""
        return (
            db.query(PericiaJogador)
    ...
    @staticmethod
    def obter_pericia_jogador_por_nome(
        db: Session, combatente_id: int, nome_pericia: str
    ) -> PericiaJogador:
    ...
    def contar_pontos_gastos(db: Session, combatente_id: int) -> int:
        ...
        resultado = (
            db.query(func.sum(PericiaJogador.custo_total))
            .filter(PericiaJogador.combatente_id == combatente_id)
            .scalar()
        )
    ...
    def contar_graduacoes(db: Session, combatente_id: int) -> int:
        ...
        resultado = (
            db.query(func.sum(PericiaJogador.graduacao))
            .filter(PericiaJogador.combatente_id == combatente_id)
            .scalar()
        )
        return resultado or 0
