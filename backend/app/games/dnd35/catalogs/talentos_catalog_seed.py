"""
Catálogo de talentos (LdJ, D&D 3.5) — fonte única para seed e importação CLI.
...
def default_json_path() -> Path:
    """
    Preferência: raiz do repositório; fallback: pasta `backend/` (deploys
    que não incluem a raiz).
    """
...
def aplicar_mapeamento_seed_antigo(
    ...
    for seed_nome, json_nome in MAPEAMENTO_SEED_PARA_JSON.items():
        ...
        if json_nome not in idx:
            avisos.append(
                f"Nome JSON não encontrado no arquivo: {json_nome!r} (seed {seed_nome!r})"
            )
            continue
...
def desativar_talentos_fora_do_catalogo(
    db: "Session", nomes_validos: set[str]
) -> tuple[int, list[str]]:
    """
...
def sincronizar_catalogo_talentos_desde_json(
    ...
    *,
    ...
    if not path.is_file():
        logger.warning(
            "Catálogo JSON não encontrado em %s — talentos não sincronizados.",
            path,
        )
        return False
    ...
    if remover_legado:
        removidos, avisos_legado = desativar_talentos_fora_do_catalogo(
            db, nomes_catalogo(rows)
        )
