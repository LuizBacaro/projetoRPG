...
class EquipamentoService:
    ...
    def criar_equipamento(self, equipamento: EquipamentoCreate) -> Equipamento:
        ...
        if equipamento_existente:
            if equipamento_existente.deleted_at is not None:
                return EquipamentoRepository.restaurar_equipamento(
                    self.db, equipamento_existente, equipamento
                )
    ...
    def atualizar_equipamento(
        ...
        db_equipamento = EquipamentoRepository.obter_equipamento(
            self.db, equipamento_id
        )

        if not db_equipamento:
            return None

        return EquipamentoRepository.atualizar_equipamento(
            self.db, equipamento_id, equipamento_data
        )
    ...
    def adicionar_equipamento_jogador(
        ...
        equipamento = EquipamentoRepository.obter_equipamento(
            self.db, equipamento_jogador.equipamento_id
        )
        if not equipamento:
            raise ValueError(
    ...
    def obter_equipamentos_jogador(
        ...
        equipamentos_jogador = (
            EquipamentoJogadorRepository.obter_equipamentos_jogador_detalhado(
    ...
    def atualizar_quantidade_equipamento(
        ...
        if quantidade == 0:
            EquipamentoJogadorRepository.remover_equipamento(
                self.db, combatente_id, equipamento_id
            )
            return None
