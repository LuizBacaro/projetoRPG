"""
Service de Campanha (D&D 3.5)

Localização: este módulo pertence ao pacote `app.games.dnd35.services`.
Existe um shim em `app.services.campanha_service` que re-exporta a
classe durante a reorganização multi-jogo.
"""
...
class CampanhaService:
    ...
    def associar_personagens(
        self, campanha_id: int, mestre_id: int, personagem_ids: List[int]
    ) -> Campanha:
    ...
    def _substituir_personagens(
        ...
        atuais = list(
            self.obter_por_id_mestre(campanha_id, mestre_id).personagens or []
        )
        ...
        self._associar_personagens(
            campanha_id, mestre_id, personagem_ids or [], commit=False
        )
        commit_with_rollback(self.campanha_repository.db)
