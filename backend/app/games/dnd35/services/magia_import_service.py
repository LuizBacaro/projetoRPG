"""Importação em lote de magias via Excel (D&D 3.5).

Localização: `app.games.dnd35.services.magia_import_service`. Shim em
`app.services.magia_import_service` durante a reorganização multi-jogo.
"""
...
class MagiaImportService:
    ...
    def confirmar_importacao(self, import_id: str) -> dict[str, Any]:
        ...
        if rows is None:
            raise HTTPException(
                status_code=404, detail="Importação expirada ou inexistente"
            )
    ...
    def _ler_xls(self, content: bytes) -> list[tuple[int, dict[str, Any]]]:
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Dependência xlrd não instalada"
            ) from exc
    ...
    def _normalizar_e_validar_linha(
        self, line_number: int, raw: dict[str, Any]
    ) -> tuple[dict[str, Any] | None, list[MagiaImportErro]]:
        erros: list[MagiaImportErro] = []
        ...
        classes_niveis = self._parse_classes_niveis(
            classes_niveis_raw, line_number, erros
        )
        componentes = self._normalizar_componentes(
            val("componentes"), line_number, erros
        )
        ...
        pagina_referencia = self._to_int(
            val("pagina_referencia"), line_number, "pagina_referencia", erros
        )
    ...
    def _parse_classes_niveis(
        self, value: str, line_number: int, erros: list[MagiaImportErro]
    ) -> list[dict[str, Any]]:
        if not value:
            erros.append(
                self._erro(
                    line_number,
                    "classes_niveis",
                    "Informe ao menos uma classe com nível",
                )
            )
            return []
        ...
        for part in value.split(";"):
            ...
            if ":" not in trecho:
                erros.append(
                    self._erro(
                        line_number, "classes_niveis", f"Formato inválido: {trecho}"
                    )
                )
                continue
            ...
            if classe not in CLASSES_VALIDAS:
                erros.append(
                    self._erro(
                        line_number, "classes_niveis", f"Classe inválida: {classe_raw}"
                    )
                )
                continue
            ...
            try:
                ...
                erros.append(
                    self._erro(
                        line_number,
                        "classes_niveis",
                        f"Nível inválido para {classe_raw}",
                    )
                )
                continue
            if nivel < 0 or nivel > 9:
                erros.append(
                    self._erro(
                        line_number,
                        "classes_niveis",
                        f"Nível fora do intervalo 0-9 para {classe_raw}",
                    )
                )
                continue
            if classe in vistos:
                erros.append(
                    self._erro(
                        line_number, "classes_niveis", f"Classe duplicada: {classe}"
                    )
                )
                continue
        ...
        if invalid:
            erros.append(
                self._erro(
                    line_number,
                    "componentes",
                    f"Componente inválido: {', '.join(invalid)}",
                )
            )
            return None
        unique = []
    ...
    def _to_int(
        self, value: str, line_number: int, field: str, erros: list[MagiaImportErro]
    ) -> int | None:
        if not value:
            return None
        try:
        ...
        if parsed <= 0:
            erros.append(
                self._erro(line_number, field, "Valor deve ser maior que zero")
            )
            return None
        return parsed
    ...
    def _armazenar_draft(self, rows: list[dict[str, Any]]) -> str:
        ...
        with self._lock:
            self._drafts[import_id] = ImportDraft(
                ...
                + timedelta(minutes=IMPORT_TTL_MINUTES),
            )
        return import_id
