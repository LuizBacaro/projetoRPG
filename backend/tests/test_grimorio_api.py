...
def test_grimorio_lista_paginada_preserva_corpo_e_headers(grimorio_db):
    ...
    listar_completo = client.get(
        f"/api/v1/grimorio/{combatente.id}", params={"classe": "MAGO"}
    )
...
def test_grimorio_lista_filtra_por_campos_e_magia_ids(grimorio_db):
    ...
    assert [item["magia_id"] for item in filtrar_nome_magia.json()] == [
        magia_evocacao.id
    ]
    ...
    assert [item["magia_id"] for item in filtrar_nome_escola.json()] == [
        magia_transmutacao.id
    ]
    ...
    assert [item["magia_id"] for item in filtrar_nome_descricao.json()] == [
        magia_ilusao.id
    ]
    ...
    assert [item["magia_id"] for item in filtrar_nome_anotacoes.json()] == [
        magia_transmutacao.id
    ]
    ...
    assert [item["magia_id"] for item in filtrar_nivel.json()] == [
        magia_transmutacao.id
    ]
    ...
    assert [item["magia_id"] for item in filtrar_componentes.json()] == [
        magia_evocacao.id
    ]
    ...
    assert [item["magia_id"] for item in filtrar_favorita.json()] == [
        magia_transmutacao.id
    ]
    ...
    filtrar_ids_csv = client.get(
        ...
        params={
            "classe": "MAGO",
            "magia_ids": f"{magia_evocacao.id},{magia_ilusao.id}",
        },
    )
    assert filtrar_ids_csv.status_code == 200
    assert {item["magia_id"] for item in filtrar_ids_csv.json()} == {
        magia_evocacao.id,
        magia_ilusao.id,
    }
...
def test_grimorio_troca_magia_bloqueia_nivel_invalido(grimorio_db):
    ...
    magia_removida = _criar_magia(db, classe="FEITICEIRO", nivel=1)
    magia_nova = Magia(
        nome="Magia Nova Alta FEITICEIRO",
        nivel=2,
        classe="FEITICEIRO",
        ativo=True,
        descricao="desc",
    ...
    assert (
        "nível" in troca.json()["detail"].lower()
        or "nivel" in troca.json()["detail"].lower()
    )
...
def test_grimorio_auto_adiciona_magias_por_nivel_para_clerigo(grimorio_db):
    ...
    magia_l2 = Magia(
        nome="Magia Clerigo 2", nivel=2, classe="CLERIGO", ativo=True, descricao="desc"
    )
    magia_l3 = Magia(
        nome="Magia Clerigo 3", nivel=3, classe="CLERIGO", ativo=True, descricao="desc"
    )
    ...
    listar = client.get(
        f"/api/v1/grimorio/{combatente.id}", params={"classe": "CLERIGO"}
    )
    assert listar.status_code == 200
    itens = listar.json()
...
def test_grimorio_lista_clerigo_com_parametro_acentuado(grimorio_db):
    ...
    listar = client.get(
        f"/api/v1/grimorio/{combatente.id}", params={"classe": "Clérigo"}
    )
    assert listar.status_code == 200
    itens = listar.json()
    ids = {item["magia_id"] for item in itens}
...
def test_grimorio_auto_adicao_nao_duplica_em_listagens_repetidas(grimorio_db):
    ...
    primeira = client.get(
        f"/api/v1/grimorio/{combatente.id}", params={"classe": "DRUIDA"}
    )
...
def test_grimorio_notificacao_magias_adicionadas_inclui_nomes(grimorio_db):
    ...
    magia = Magia(
        nome="Luz Sagrada", nivel=1, classe="CLERIGO", ativo=True, descricao="desc"
    )
    db.add(magia)
    db.flush()
...
def test_grimorio_bloqueia_magia_acima_do_nivel_conjuravel(grimorio_db):
    ...
    assert (
        "nível máximo" in resp.json()["detail"].lower()
        or "nivel máximo" in resp.json()["detail"].lower()
    )
...
def test_grimorio_mago_bloqueia_adicao_acima_nivel_conjuravel(grimorio_db):
    ...
    magia_n2 = Magia(
        nome="Mago N2", nivel=2, classe="MAGO", ativo=True, descricao="desc"
    )
    db.add(magia_n2)
    db.flush()
...
def test_grimorio_notificacao_selecao_pendente_inclui_niveis(grimorio_db):
    ...
    magia_n0 = Magia(
        nome="Magia FEITICEIRO N0", nivel=0, classe="MAGO", ativo=True, descricao="desc"
    )
    magia_n1 = Magia(
        nome="Magia FEITICEIRO N1", nivel=1, classe="MAGO", ativo=True, descricao="desc"
    )
    magia_n2 = Magia(
        nome="Magia FEITICEIRO N2", nivel=2, classe="MAGO", ativo=True, descricao="desc"
    )
...
def test_grimorio_notificacao_conversao_divina_por_excecao_wee_jas(
    monkeypatch, grimorio_db
):
    db, db_factory = grimorio_db
    combatente = _criar_combatente(db, classe="Clerigo")
    combatente.alinhamento = "Leal e Neutro"
...
def test_grimorio_clerigo_auto_adiciona_apenas_dominios_escolhidos_no_nivel(
    grimorio_db,
):
    db, db_factory = grimorio_db
    combatente = _criar_combatente(db, classe="Clerigo")
...
def test_grimorio_clerigo_adiciona_magia_de_dominio_ao_subir_nivel(grimorio_db):
    ...
    item_n3 = next(
        item for item in itens_segunda if item["magia_id"] == magia_bem_n3.id
    )
...
def test_grimorio_bardo_bloqueia_adicao_acima_nivel_conjuravel(grimorio_db):
    ...
    magia_n3 = Magia(
        nome="Bardo N3", nivel=3, classe="BARDO", ativo=True, descricao="desc"
    )
    db.add(magia_n3)
    db.flush()
...
def test_grimorio_druida_bloqueia_adicao_manual_por_dominio_contrario_ao_alinhamento(
    grimorio_db,
):
    db, db_factory = grimorio_db
    combatente = _criar_combatente(db, classe="Druida")
...
def test_grimorio_paladino_bloqueia_adicao_manual_por_dominio_contrario_ao_alinhamento(
    grimorio_db,
):
    db, db_factory = grimorio_db
...
def test_grimorio_clerigo_bloqueia_dominio_oposto_mesmo_nao_sendo_magia_de_dominio(
    grimorio_db,
):
    db, db_factory = grimorio_db
    combatente = _criar_combatente(db, classe="Clerigo")
    ...
    assert resp.status_code == 400
    assert (
        "domínio oposto" in resp.json()["detail"].lower()
        or "dominio oposto" in resp.json()["detail"].lower()
    )
...
def test_grimorio_matriz_alinhamento_tendencia_divina(
    grimorio_db, alinhamento, permitidas
):
    db, db_factory = grimorio_db
    combatente = _criar_combatente(db, classe="Clerigo")
    combatente.nivel = 3
    ...
    magias = {
        ...
        "ORDEM": Magia(
            nome=f"Matriz Ordem {alinhamento}",
            nivel=1,
            classe="CLERIGO",
            ativo=True,
            descricao="desc",
        ...
        "CAOS": Magia(
            nome=f"Matriz Caos {alinhamento}",
            nivel=1,
            classe="CLERIGO",
            ativo=True,
            descricao="desc",
