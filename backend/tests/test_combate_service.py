...
class TestCombateService:
    ...
    def test_iniciar_combate_sucesso(
        self, service, mock_combate_repo, mock_combatente_repo
    ):
        ...
        mock_combatente_repo.ordenar_por_iniciativa.return_value = sorted(
            combatentes, key=lambda c: c.iniciativa, reverse=True
        )

        combate_criado = Combate(
            id=1, combatentes_ids=[2, 1], turno_atual=0, ativo=True
        )
        ...
        assert resultado.combatentes_ids == [
            2,
            1,
        ]  # Ordenado por iniciativa (Lyra 18, Theron 15)
    ...
    def test_avancar_turno(self, service, mock_combate_repo, mock_combatente_repo):
        ...
        # Arrange
        combate_mock = Combate(
            id=1, combatentes_ids=[1, 2, 3], turno_atual=0, ativo=True
        )
    ...
    def test_avancar_turno_falha_com_versao_desatualizada(
        self, service, mock_combate_repo
    ):
        combate_mock = Combate(id=1, combatentes_ids=[1, 2], turno_atual=0, ativo=True)
        mock_combate_repo.get_ativo.return_value = combate_mock
