...
def test_magias_preparadas_max_full_caster_nao_regride(classe, attr, valor_attr, nivel, esperado):
    db = SessionLocal()
    try:
        kwargs = {
            "tipo": "jogador",
...
def test_paladino_prepara_magias_car_nivel_metade():
    ...
    try:
        ...
        magia = (
            db.query(Dnd5eMagia).filter(Dnd5eMagia.nivel == 1, Dnd5eMagia.ativo.is_(True))
            .first()
        )
        if not magia:
            magia = Dnd5eMagia(
